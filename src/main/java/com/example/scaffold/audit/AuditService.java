...
public class AuditService {
    ...
    public void logEvent(String entityType, Long entityId, String eventType, Object details, Long userId) {
        try {
            ...
            auditEventRepository.save(auditEvent);
            log.debug("Audit event logged: {} {} for entity {}:{} by user {}",
                    eventType, entityType, entityType, entityId, userId);

        } catch (Exception e) {
            log.error("Failed to log audit event for {}:{} - {} by user {}",
                    entityType, entityId, eventType, userId, e);
        }
    }
